...
router.patch('/:username', authUser, requireLogin, async function( // FIXES BUG #8
  req,
  res,
  next
) {
  try {
    ...
    if (fields.username || fields.password || fields.admin) {
      throw new ExpressError('Can only update the following fields: first_name, last_name, phone, email.', 401);
    }
