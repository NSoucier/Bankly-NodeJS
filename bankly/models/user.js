...
/** Register user with data. Returns new user data. */
  ...
  static async register({username, password, first_name, last_name, email, phone, admin}) {
    ...
    const result = await db.query( // FIXES BUG #3
      `INSERT INTO users 
          (username, password, first_name, last_name, email, phone, admin) 
        VALUES ($1, $2, $3, $4, $5, $6, $7) 
        RETURNING username, first_name, last_name, email, phone, admin`, // FIXES BUG #2
      [
        username,
        hashedPassword,
        first_name,
  ...
  static async authenticate(username, password) {
    ...
    if (user && (await bcrypt.compare(password, user.password))) {
      delete user.password; // FIXES BUG #2
      return user;
    } else {
  ...
  static async getAll() { // FIXES BUG #4
    const result = await db.query( // FIXES BUG #5
      `SELECT username,
                first_name,
                last_name
            FROM users 
  ...
  static async get(username) {
    ...
    if (!user) {
      throw new ExpressError('No such user', 404); // FIXES BUG #7
    }
